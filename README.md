...
探討如何優化 Celery 分散式任務處理的日誌系統，目的是提升可追蹤性和閱讀性。

## 1. 識別日誌追蹤的挑戰

在面對原始的 Celery 日誌時，我們遇到了以下幾個主要挑戰：

- **任務追蹤困難**：從大量日誌中快速識別特定任務的日誌變得困難。
- **缺乏清晰的上下文連結**：日誌條目間缺少清晰的相互關聯，使得追蹤任務間的依賴和觸發關係變得困難。
...
為了解決上述問題，引入 [asgi-correlation-id](https://github.com/snok/asgi-correlation-id)，這是一種為每個任務生成唯一識別碼的工具，能夠顯著提升日誌的可追蹤性。
...
在 Celery 的 `celery.worker.strategy.py` 的 `default` 函數中，`LOG_RECEIVED` 日誌記錄後才發出 `task_received` 信號。這導致日誌輸出與信號處理之間的不同步，影響了日誌的完整性和任務追蹤能力。
